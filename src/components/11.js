...
useEffect(() => {
  if (actionQueue.length > 0 && !queueInProgress) {
    const nextAction = actionQueue.shift();
    nextAction();
    setQueueInProgress(true);
  }
}, [actionQueue, queueInProgress]);
...
const addToQueue = (action) => {
  ...
  useEffect(() => {
	...
	}, [currentSlide, isAutoPlayPaused, isAnimationComplete]);

	const nextSlide = () => {
		if (isAnimationComplete && !queueInProgress) {
